...
s = s[:start] + '''    private fun refreshList() {
        ...
        fun renderChunk(startIndex: Int) {
            ...
            for (i in startIndex until endIndex) {
                container.addView(observationCard(items[i]), LinearLayout.LayoutParams(-1, dp(150)).apply { setMargins(0, 0, 0, dp(12)) })
            }
...
one('loadImage(this, uri)\n                    setOnClickListener { loadImage(detailImage, uri) }', 'loadImage(this, uri, 180)\n                    setOnClickListener { loadImage(detailImage, uri, 960) }', 'detail thumb target')
one('loadImage(this, formImageUri)', 'loadImage(this, formImageUri, 900)', 'form image target')
one('formImageView?.let { loadImage(it, formImageUri) }', 'formImageView?.let { loadImage(it, formImageUri, 900) }', 'form refresh target')
...
one('wrap.addView(text("الإصدار 1.0.14", 12f, Color.rgb(150, 158, 154), false).apply { gravity = Gravity.CENTER; setPadding(0, dp(16), 0, 0) })',
    'wrap.addView(text("الإصدار 1.0.15   |   برمجة : ابوريان الغامدي", 12f, Color.rgb(150, 158, 154), true).apply { gravity = Gravity.CENTER; setPadding(0, dp(16), 0, 0) })', 'version credit')
...
s = s[:start] + '''    private fun loadImage(view: ImageView, uriString: String, targetPx: Int = 640) {
        ...
        imageExecutor.execute {
            ...
            view.post {
                ...
                if (bitmap != null) { view.clearColorFilter(); view.scaleType = ImageView.ScaleType.CENTER_CROP; view.setImageBitmap(bitmap) }
                else showImagePlaceholder(view)
            }
        }
    }
